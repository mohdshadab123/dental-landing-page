...
{ name: "Anita",    location: "Bangalore",     treatment: "Laser Treatment", rating: 5 },
  ...
  const goNext = () => {
    setActiveIndex((old) => (old + 1) % list.length);
  };

  const goPrev = () => {
    setActiveIndex((old) => (old - 1 + list.length) % list.length);
  };
    ...
    <section id="testimonials" className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center mt-8 space-x-2">
          ...
          {list.map((_, dotsIndex) => (

            <button
              key={dotsIndex}
              onClick={() => setActiveIndex(dotsIndex)}
              ...
              className={`w-2 h-2 rounded-full transition ${
                ...
                dotsIndex === activeIndex ? "bg-teal-600 w-8" : "bg-gray-300"
              }`}
            />
          ))}
        </div>
